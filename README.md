...
Download PsExec:
Download the official Sysinternals Suite from Microsoft: Download Here.
...
Right-click on the Gallery-Lock.ps1 script file.
Select "Run with PowerShell".
If prompted, approve the UAC (User Account Control) prompt to grant it administrator rights.
...
🔍 Scan & Clean: It first checks for and deletes any existing Gallery.exe files in the target locations, ensuring a clean slate.
📝 Create the Decoy: An empty, 0-byte file named Gallery.exe is created. It's harmless and takes up no space.
🛡️ Build the Fortress (ACL Hardening): This is the most critical step. The script modifies the file's Access Control List (ACL):
...
👻 Go Invisible: Finally, it sets the file attributes to Hidden and System, hiding it from standard view in File Explorer to prevent accidental discovery or tampering.
...
Take Ownership of the file. Replace the path with the correct one.
For the user file:
Generated cmd
...
Use code with caution.
Cmd
Reset Permissions to inherit from the parent folder.
For the user file:
Generated cmd
...
Use code with caution.
Cmd
You can now delete the file normally.
Generated powershell
...
🗑️ I can't delete the file, even as an Admin!	This is the script working correctly! It's designed to block everyone, including you. Follow the steps in the How to Undo section to remove it.
...
📜 License
This project is open-source and distributed under the MIT License. You are free to use, share, and modify it.
